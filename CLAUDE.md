...
**Single-link entry point.** `pipeline.add_job_from_url(url, template, tailor=True)` builds a job from a pasted URL: it scrapes the JD off the page, calls `engine.extract_job_meta(jd)` (one LLM call → title/company/location, since a pasted link has no job-card metadata), and upserts the job **flagged `source="manual"`** under the **same `_job_key` id scheme** as the scraper (re-pasting updates, never duplicates). Returns `{"error": ...}` if the page yields too little text (scrape-blocked sites). Two endpoints drive it:
- `POST /api/jobs/add-url` (`tailor=False`, **synchronous**) — powers the dedicated **Add Job** dashboard tab: adds the manual job *without* tailoring and returns the detected title/company. The user then tailors on demand via the per-job **Run via pipeline** button (`POST /api/applications/{id}/process` → `run_single_job`).
- `POST /api/pipeline/run-url` (`tailor=True`, `BackgroundTasks` behind `_pipeline_running`) — one-shot add **and** tailor. Kept as an API capability; the UI no longer calls it.
...
- **Summary voice is auto-matched to the JD, not picked.** `pdf_gen.py` only ever renders `summary_templates["default"]`. `tailor_resume_scored` calls `engine.select_summary_angle(jd, requirements)` — a deterministic signal-phrase scorer over the 5 voices (default/executive/engineering/ai_ml/management) — and `tailor_resume` **seeds the chosen voice's text into the `default` slot** before tailoring, so the model tailors the right angle and the PDF renders it. The angle is stored in `match_report["summary_angle"]` and as the application's `template`. Don't "fix" pdf_gen to render a non-default key; the seed approach is intentional.
- **`pdf_gen.py` renders an optional `awards` block** (between summary and experience) only when the resume has an `awards` field — so applications tailored before the field existed stay clean. `data/master/resume.json` mirrors the real `LiborCevelik2026.pdf`: 5 experience entries (incl. Studio Mirage 2005–09), `awards`, education, certs. It's gitignored (backup `.bak`). **Tailored resumes are stored snapshots — editing the master does not change existing applications until they're re-tailored** (`scripts.regen_resumes`).
- **`search_location` is multi-value.** `pipeline.run_pipeline` splits it on commas and searches each location separately (the scrapers take one location per request) — e.g. `Los Angeles, Remote, California` → three searches, deduped by stable id. Keywords are split the same way. `target_companies` only affects priority ranking (and the hardcoded `scrape_target_companies` career-page dict, which doesn't include every target).
